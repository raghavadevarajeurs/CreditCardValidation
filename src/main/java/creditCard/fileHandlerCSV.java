...
public class fileHandlerCSV extends fileCategorizer {
            ...
            */
            ...
            while ((line = fileReader.readLine()) != null && line != "") {
                ...
                if (tokens.length > 0 && tokens.length != 1) {
                    ...
                    creditCardInput inputRow;
                    if(super.isDateValid(tokens[CC_EXP_DATE_IDX])) {
                        inputRow = new creditCardInput(tokens[CC_NUMBER_IDX], tokens[CC_EXP_DATE_IDX], tokens[CC_HOLDER_NAME]);
                    }
                    else {
                        System.out.println(tokens[CC_EXP_DATE_IDX] +" -Date format not supported");
                        inputRow = new creditCardInput(tokens[CC_NUMBER_IDX], "Unsupported Date Format", tokens[CC_HOLDER_NAME]);
                    }
    ...
    public void writeFile(String fileName, List<creditCardInput> creditCardList) {
        ...
        try {
            ...
            for( creditCardInput inputRow : creditCardList ){
                System.out.println(inputRow.toString());
                fileWriter.append(inputRow.getCardNumber());
